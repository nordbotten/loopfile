...
- **Trust prompt (#7):** a red-on-black DANGER banner, then a summary from the manifest: the source, the full SHA, each step's kind, harness and model, the first line of each `run`, and full agent `args`. Then an `unpack` line to read the full text, then a select: trust the repo, trust the owner, or Deny, which is the default and writes nothing.
