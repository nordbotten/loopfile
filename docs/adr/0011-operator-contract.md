...
- **Terminal (#191, #69):** the two commands that need a terminal are `status` with no run ID and `cancel <loopid>` with no mode. `status` lists runs and loops to pick from; `cancel` asks whether to stop the current run now or after it ends. Without a terminal, bare `status` refuses with `no_terminal`, as does `cancel <loopid>` without `--now` or `--after-run`. Every other command works the same with or without one, except that the launch path and `resume` block silently until the run ends when they have no `-d`.
...
- **Manifest from stdin (#193):** `loopfile -` and `loopfile check -` read a thin manifest from stdin, so an agent writes no scratch file. A manifest with assets still needs a directory. `loopfile check <source>` validates the manifest and the names of any `--input` flags given, not the environment. `check` exiting `0` means launch will not refuse the manifest itself; launch still checks for missing input values (#155). A failed launch prints one `error:` line per problem.
...
- **Result (#195):** `loopfile result <runid>` reads what a run produced from `events.jsonl` and the run folder, with no run owner: the end reason, last outcome, workspace mode and path, Target folder when it has one, branch and base commit for an isolate worktree, and declared inputs and outputs. Values are inline up to 64 KiB, then a path. `result --json` is a versioned format (ADR 0006).
...
- **Removal (#197):** run folders are deleted only by `remove <runid>` or `prune`; a successful run attempts to remove its isolate worktree, which stays if Git refuses. They never delete the run branch. A `here` run's Target folder is left alone. Copy and empty workspaces are removed directly; an isolate worktree with uncommitted changes refuses removal unless `remove --force`. `prune` never forces (ADR 0003, 0014).
