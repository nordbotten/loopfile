...
- **Target folder:** in `here`, the launch folder; otherwise the Git top level when the launch folder is in a Git repository and Git is available, or the launch folder. The run records this as `targetFolder`.
...
- **Selection and default:** an optional top-level `workspace:` field in the Manifest selects a supported mode (`isolate` or `here` in this release). `--workspace <mode>` overrides it for a launch or a loop. The default is `isolate`, so a Loopfile with no new setting keeps its existing behavior.
