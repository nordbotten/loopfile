...
- **Unknown metrics:** every status field is always present. For a harness metric such as tokens, cost or tool count, `null` means unknown. A number, including `0`, is a value the harness reported. A harness adapter never estimates a value. Which fields exist is decided in #46.
