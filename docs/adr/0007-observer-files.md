...
- **What the activity log gets:** step start and end, the start of each Ralph iteration (nordbotten/loopfile#241), the route taken, the outcome, each agent tool call from the harness (the tool name and a short target, for example `edit src/x.ts`), each data or result command (for example `data get review.md`), and short progress text from the harness.
