...
- **An added event field is an addition:** the optional `metrics` field on `attempt.ended` is an addition, so the event format version stays `1`. The optional `remote` record on `run.created` and in `status.json` is an addition too, so the event and status versions stay `1` (nordbotten/loopfile#9, ADR 0013).
